...
def test_categorical_column_with_identity():
    color_data = {'color': [[2], [5], [-1], [0], [5], [-1]]}
    builder = _LazyBuilder(color_data)
    color_column = feature_column.categorical_column_with_identity('color', 100000, default_value=0)
    color_column_tensor = color_column._get_sparse_tensors(builder)
    with tf.Session() as session:
        session.run(tf.global_variables_initializer())
        session.run(tf.tables_initializer())
...
test_categorical_column_with_identity()


def test_embedding_column_with_hash_bucket():
    color_data = {'color': [[2, 2], [2, 3], [2, 4], [0, -1], [0, -2], [0, 0]]}
    builder = _LazyBuilder(color_data)
    color_column = feature_column.categorical_column_with_hash_bucket('color', 7, dtype=tf.int32)
    color_column_tensor = color_column._get_sparse_tensors(builder)
    with tf.Session() as session:
...
def test_shared_embedding_column_with_hash_bucket():
    ...
    color_column_embed = feature_column.shared_embedding_columns([color_column2, color_column], 2, combiner='sum')
