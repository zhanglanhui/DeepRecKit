...
class FCDNN(Layer):
    def __init__(self, hidden_units, activation='relu', l2_reg=0, dropout_rate=0, use_bn=False, output_activation=None,
        ...
        self.dropout_layers = None
        self.activation_layers = None
        super(FCDNN, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        for i in range(len(self.hidden_units)):
            if i > 0:
                last_input_size += self.hidden_units[i - 1]
            self.kernels.append(
                self.add_weight(name='kernel' + str(i),
                                shape=(last_input_size, self.hidden_units[i]),
                                initializer=glorot_normal(seed=self.seed),
                                regularizer=l2(self.l2_reg),
                                trainable=True))
            self.bias.append(
        ...
        if self.output_activation:
            self.activation_layers[-1] = activation_layer(self.output_activation)
        super(FCDNN, self).build(input_shape)  # Be sure to call this somewhere!
    ...
    def call(self, inputs, training=None, **kwargs):
        ...
        for i in range(len(self.hidden_units)):
            ...
            if i == len(self.hidden_units) - 1:
                ...
                deep_input = tf.concat([fc, deep_input], axis=-1)
        return deep_input
    ...
    def get_config(self, ):
        config = {'activation': self.activation, 'hidden_units': self.hidden_units,
                  'l2_reg': self.l2_reg, 'use_bn': self.use_bn, 'dropout_rate': self.dropout_rate,
                  'output_activation': self.output_activation, 'seed': self.seed}
        base_config = super(FCDNN, self).get_config()
        base_config.update(config)
        return base_config
